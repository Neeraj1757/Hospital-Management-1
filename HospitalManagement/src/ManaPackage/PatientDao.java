...
public class PatientDao implements PatientDaoInterface {
	...
	public boolean insertPatient(Patient p) {
		...
		try {
			...
			// Inserting Data
			String query = "insert into patient_details(patient_name,age,sex,address) value(?,?,?,?)";
			/*
	...
	public boolean delete(int id) {
		...
		try {
			Connection con = DBConnection.createConnection();
			String query = "delete from patient_details where patient_id=" + id;
			PreparedStatement pst = con.prepareStatement(query);
			pst.executeUpdate();
			flag = true;
	...
	public boolean update(int id, String update, int ch, Patient p) {
		...
		try {
			if (choice == 1) {
				...
				// DBConnection class
				Connection con = DBConnection.createConnection();
				String query = "update patient_details set patient_name=? where patient_id=?";
	...
	public boolean update1(int id, int update, int ch, Patient p) {
		...
		try {
			if (choice == 2) {
				...
				// DBConnection class
				Connection con = DBConnection.createConnection();
				String query = "update patient_details set age=? where patient_id=?";
				PreparedStatement ps = con.prepareStatement(query);
	...
	public boolean showPatientById(int id) {
		...
		try {
			...
			// DBConnection class
			Connection con = DBConnection.createConnection();
			String query = "select * from patient_details where patient_id=" + id;
			Statement stmt = con.createStatement();
